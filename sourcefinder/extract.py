...
class ParamSet(MutableMapping):
    ...
    def calculate_errors(self, noise, max_pix_variance_factor,
        ...
        max_pix_variance_factor : float
            Factor to account for the additional uncertainty introduced by the
            fudge_max_pix correction.
    ...
    def _error_bars_from_moments(self, noise, max_pix_variance_factor,
                                 correlation_lengths, threshold):
        """
...
def source_profile_and_errors(data, threshold, noise,
    ...
    fudge_max_pix_factor : float
        ...
        pixel value.
    max_pix_variance_factor : float
        Take account of additional variance induced by the maximum pixel
        method, on top of the background noise.
    beamsize : float
...
def insert_sources_and_noise(some_image, noise_map, inds, labelled_data, label,
    ...
    fitting.moments_enhanced. Simultaneously calculate the minimum width of each
    island; when determining source properties this is an important integer, 
    i.e. with sufficient width all six Gaussian parameters can be calculated in 
    a robust manner.
    ...
    None
        ...
        'guvectorize() functions don’t return their result value: they take it as an
        array argument, which must be filled in by the function'. In this case
...
def source_measurements_pixels_and_celestial_vectorised(num_islands, npixs,
    ...
    'fitting.moments' and 'extract.Detection._physical_coordinates' but with a
    data layout suitable for vectorisation.
    ...
    fudge_max_pix_factor : float
        ...
        beam is appropriate.
    max_pix_variance_factor : float
        Takes account of the additional uncertainty introduced by
        fudge_max_pix. Can probably be removed since its effect is negligible in
        all sensible cases.
    beam : tuple
    ...
    reduced_chisq : np.ndarray
        ...
        calculated in fitting.goodness_of_fit.
    """
