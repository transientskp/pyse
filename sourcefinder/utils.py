...
def maximum_pixel_method_variance(semimajor, semiminor, theta):
    """Estimate variance for peak spectral brightness.

    When we use the maximum pixel method, with a statistical correction
    fudge_max_pix, which, strictly speaking, is only valid for a circular clean
    beam, there should be no bias. This requires that the peaks of the
    Gaussians are randomly distributed. In practice, however, they could be
    relatively close to the centres of the pixels due to selection effects from
    detection thresholds.

    Disregarding the latter effect and noise, we can compute the
    variance of the maximum pixel method with the fudge_max_pix statistical
    correction by integrating (the true spectral brightness -
    the average true spectral brightness) ^ 2 =
    (the true spectral brightness - fudge_max_pix) ^ 2
    over the pixel area and dividing by the pixel area ( = 1).  This
    is just equal to integral of (the true spectral brightness ^ 2) over the
    pixel area - fudge_max_pix^2.
    ...
    -------
    variance: float
        The estimated variance from the fudge_max_pix statistical correction.

    Notes
    -----
    This factor is very close to 1 in all non-pathological cases. Perhaps
    it should be omitted from PySE.
    """
